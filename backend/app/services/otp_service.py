...
class OTPService:
    ...
    def _check_rate_limit(self, identifier: str, channel: str) -> bool:
        ...
        if self.redis_client.get(key):
            return False
        self.redis_client.setex(key, self.settings.otp_rate_limit_seconds, "1")
        return True

    def _expires_at(self) -> datetime:
        return datetime.now(timezone.utc) + timedelta(minutes=self.settings.otp_expire_minutes)
    ...
    def send_email_otp(self, email: str) -> tuple[str, int]:
        ...
        return otp_code, self.settings.otp_expire_minutes * 60
    ...
    def send_mobile_otp(self, mobile_number: str) -> tuple[str, int, bool]:
        ...
        sms_delivered = self.sms_service.send_otp_sms(mobile_number, otp_code)
        if not sms_delivered:
            logger.info("Mobile OTP for +91%s: %s (SMS not delivered)", mobile_number, otp_code)
        return otp_code, self.settings.otp_expire_minutes * 60, sms_delivered

    def verify_email_otp(self, email: str, otp_code: str) -> bool:
        otp = self.otp_repo.get_latest_unverified(email=email)
    ...
    def verify_mobile_otp(self, mobile_number: str, otp_code: str) -> bool:
        otp = self.otp_repo.get_latest_unverified(mobile_number=mobile_number)
        if not otp or otp.otp_code != otp_code:
            return False
        if self.otp_repo.is_expired(otp):
            return False
        self.otp_repo.mark_verified(otp)
        self.redis_client.setex(f"mobile_verified:{mobile_number}", 1800, "1")
        return True

    def is_mobile_verified(self, mobile_number: str) -> bool:
        return bool(self.redis_client.get(f"mobile_verified:{mobile_number}"))
